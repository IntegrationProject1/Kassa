...
# Queue names
QUEUES = ['crm_user_update', 'facturatie_user_update', 'frontend_user_update']

# Routing keys
ROUTING_KEYS = {
    'crm_user_update': 'crm.user.update',
    'facturatie_user_update': 'facturatie.user.update',
    'frontend_user_update': 'frontend.user.update',
}
...
class ResPartner(models.Model):
    ...
    def publish_user_update(self, partner_data):
        """Publish user update message to RabbitMQ for multiple queues with routing keys"""
        try:
            ...
            connection.close()
            log_message("RabbitMQ connection closed. Successfully sent update message to all queues.")
            return True
